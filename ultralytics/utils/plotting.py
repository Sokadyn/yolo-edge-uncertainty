...
# TODO: implement uncertainty thresholding
@threaded
def plot_images_with_uncertainty(
    labels: Dict[str, Any],
    images: Union[torch.Tensor, np.ndarray] = np.zeros((0, 3, 640, 640), dtype=np.float32),
    paths: Optional[List[str]] = None,
    fname: str = "images.jpg",
    ...
    save: bool = True,
    conf_thres: float = 0.25,
    unc_thres: float = 0.75
) -> Optional[np.ndarray]:
    """
    Plot image grid with labels, bounding boxes, masks, keypoints, and uncertainty scores.
    ...
    Args:
        ...
        save (bool): Whether to save the plotted image grid to a file.
        conf_thres (float): Confidence threshold for displaying detections.
        unc_thres (float): Uncertainty threshold for displaying uncertainty scores.

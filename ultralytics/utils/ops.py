...
def calc_uncertainty(cls_logits, method='softmax-entropy', uncertainty_type='total', multi_sample=False):
    ...
    Args:
        cls_logits (torch.Tensor): Class logits tensor (raw, not activated).
    ...
    Returns:
        torch.Tensor: Uncertainty values. (batch_size, num_detectors, 1)
    """
    
    if method == 'sigmoid-complement':
        # uncertainty = 1 - max(confidence)
        cls_probs = F.sigmoid(cls_logits)
        if multi_sample:
            conf = cls_probs.mean(dim=0).max(dim=-1, keepdim=True)[0]
        else:
            conf = cls_probs.max(dim=-1, keepdim=True)[0]
        return 1.0 - conf

...
class v8DetectionLossEDLMEH:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.stride = m.stride  # model strides
        self.nc = m.nc  # number of classes
        self.no = m.nc + m.reg_max * 4 + 1 # +1 for MEH head lambda
        self.reg_max = m.reg_max
        self.device = device
    ...
    def __call__(self, preds, batch):
        ...
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl
        feats = preds[1] if isinstance(preds, tuple) else preds
        pred_distri, pred_scores, meh_lambda = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc, 1), 1
        )

        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()
        meh_lambda = meh_lambda.permute(0, 2, 1).contiguous()
